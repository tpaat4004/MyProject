...
export default {
  setup() {
    ...
    onMounted(() => {
      orderId.value = route.query.vnp_TxnRef || 'Không có mã đơn hàng';
      totalAmount.value = (parseInt(route.query.vnp_Amount, 10) || 0) / 100; // VNPay trả về số tiền nhân 100
      paymentMethod.value = route.query.vnp_CardType || 'Không rõ';
    });
